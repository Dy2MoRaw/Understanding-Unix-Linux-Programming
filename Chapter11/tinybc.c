 ...
 * 		  and pases result back to stdout
 *
 *	       +--------+                +---------+
 * 	stdin >0	>== pipetodc ====>         |
 * 	       | tynybc |                |   dc -  |
...
void be_dc( int to[2], int from[2] )
/*
 * 	set up stdin and stdout, then execl dc
 */
{
/* setup stdin from pipein */
	if( dup2(to[0], 0) == -1 )		/* copy read end to 0 */
		oops( "dc: cannot redirect stdin", 3 );
	close(to[0]);				/* moved to fd 0 */
	close(to[1]);				/* won't write here */

/* setup stdout to pipeout */
	if( dup2(from[1], 1) == -1 )		/* dupe write end to 1 */
		oops( "dc: cannot redirect stdout", 4 );
	close(from[1]);				/* moved to fd 1 */
	close(from[0]);				/* won't read from here */
